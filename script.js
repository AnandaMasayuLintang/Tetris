(function () {
  ...
  function AddNextFigure() {
    ...
    function figureRow() {
      for (var b = 0; b < 4; b++) {
        nextDisplayArr[position + element[b]].className += " pixelNextFigure";
      }
    }

    function figureErase() {
      for (var f = 0; f < 4; f++) {
        nextDisplayArr[position + element[f]].className = "pixelDisplayNext";
      }
    }
  }
  ...
  function FigureController() {
    ...
    function positionCheck(step, elem) {
      ...
      for (var l = 0; l < 4; l++) {
        if (displayArray[figurePosition + elem[l] + step] === undefined) {
          downPosition[l] = false;
          continue;
        } else downPosition[l] = true;

        if (displayArray[figurePosition + elem[l] + step].classList.length > 1) {
    ...
    function isPartYourself(elem, step, renewed) {
      ...
      for (var a = 0; a < 4; a++) {
        if (figurePosition + renewed[elem] + step === figurePosition + figureData[a]) {
          result = true;
          break;
        } else {
          result = false;
    ...
    function nextFigure() {
      ...
      objFigure.addNewFigure(newFigure.FormSelection(), newFigure.getStartPosition());
    }
  }
  ...
  function ObjectFigure() {
    ...
    this.draw = function () {
      for (var n = 0; n < 4; n++) {
        displayArray[positionFigure + figureArray[n]].className += " figurePixel";
      }
    };

    this.erase = function () {
      for (var n = 0; n < 4; n++) {
        displayArray[positionFigure + figureArray[n]].className = "pixel";
      }
    };
  ...
  function PointsUp() {
    ...
    function app(startValue, added) {
      animate({
        ...
        draw: function (progress) {
          displayPoints.innerHTML = startValue + Math.floor(progress * added);
        },
      });
    }
  }
  ...
  function User() {
    this.createUserForm = function () {
      document.getElementById("displayWindow").appendChild(userFormBlock());
    };

    this.createUser = function () {
      if (accessCreate) {
        var userName = document.querySelector(".userCreate input").value;
        menuAccess = true;

        if (userName.length < 9 && userName.length >= 3) {
          if (~userName.indexOf("<") || ~userName.indexOf(">") || ~userName.indexOf("&") || ~userName.indexOf(",")) {
            return usernameError();
          } else userNameField.innerHTML = userName;
        } else return usernameError();

        document.getElementById("displayWindow").removeChild(document.querySelector(".userCreate"));
        buttonControl = true;
        keyAccess = true;
        objFigure.addNewFigure(newFigure.FormSelection(), newFigure.getStartPosition());
        figureCont.start();
    ...
    function usernameError() {
      ...
      messageText.innerHTML = '<span>Nickname should not contain characters: "<,> /", and not be shorter than 3 and not longer than 9 characters</span>';
    ...
    function windowErrorClose() {
      ...
      document.getElementById("displayWindow").removeChild(document.querySelector(".errorMessage"));
    }
  }
  var user = new User();
  ...
  function Menu() {
    var classArrRecords = ["firstPlace", "secondPlace", "thirdPlace", "fourthPlace", "fifthPlace"];
    ...
    this.showMenu = function () {
      if (menuAccess) {
        ...
        var closeButton = document.querySelector(".listMenu div:first-child");
        closeButton.onclick = menu.closeMenu;

        var newGameButton = document.querySelector(".listMenu div:nth-child(2)");
    ...
    this.closeMenu = function () {
      ...
      setTimeout(function () {
        document.getElementById("displayWindow").removeChild(menuDisplay);
    ...
    function homeMenu() {
      ...
      setTimeout(function () {
        menuDisplay.removeChild(document.getElementsByClassName("listMenu")[0]);
      }, 900);
    }
  ...
  function Preservation() {
    this.addSave = function () {
      ...
      return saveArrRefresh([date, points, figureControllerSave, objectFigureSave, displaySaveArr]);
    };
    ...
    function getDate() {
      ...
      return "<span>" + day + "." + month + "." + year + "</span><span>" + hours + ":" + minutes + ":" + seconds + "</span>";
    }
    ...
    this.installSave = function () {
      ...
      if (gameEnd) {
        document.getElementById("displayWindow").removeChild(document.querySelector(".gameOver"));
